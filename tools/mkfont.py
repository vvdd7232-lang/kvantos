#!/usr/bin/env python3
"""Builds an 8x16 VGA font in the CP866 layout from system PSF fonts.
Result: kernel/font8x16.c (an array of 256*16 bytes)."""
...
def load_psf(path):
    ...
    if data[:2] == PSF1_MAGIC:
        ...
        raise ValueError('not a PSF file: ' + path)
...
def glyph_for(font, cp, target_h=16):
    ...
    if h == target_h:
        return rows
    if h < target_h:                       # vertical centring
        pad = target_h - h
        top = pad // 2
...
# --- CP866: 0x00-0x7F ASCII, 0x80-0xAF Cyrillic (part), 0xB0-0xDF box
# --- drawing, 0xE0-0xEF Cyrillic tail, 0xF0.. Yo and the rest
CP866 = {}
for i in range(0x20, 0x7F):
    CP866[i] = i
# A..Ya -> 0x80..0x9F ; a..p -> 0xA0..0xAF
for k in range(0x20):
...
CP866[0xF0] = 0x401   # capital Yo
CP866[0xF1] = 0x451   # small yo
CP866[0xFC] = 0x2116  # numero sign
CP866[0xF8] = 0xB0    # degree sign
CP866[0xFA] = 0xB7    # middle dot
CP866[0xFD] = 0xA4    # currency sign
...
CP866.update(BOX)
# arrows and friends in the spare cells of the control zone (for the UI)
CP866[0x01] = 0x263A
CP866[0x03] = 0x2665
CP866[0x04] = 0x2666
CP866[0x0F] = 0x263C
...
for p in SRC:
    if os.path.exists(p):
        try:
            ...
            print('skipped', p, e, file=sys.stderr)
if not fonts:
    sys.exit('no source fonts found')

# Glyphs drawn by hand (the source fonts do not carry them)
MANUAL = {
...
with open(os.path.join(os.path.dirname(__file__), '..', 'kernel', 'font8x16.c'), 'w') as f:
    f.write('/* Generated by tools/mkfont.py - VGA 8x16, CP866 layout */\n')
...
print('done; empty glyphs:', len(missing))
if missing[:10]:
