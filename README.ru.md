...
|---|---|---|---|
| **FAT32** | да | да | длинные имена (VFAT), подкаталоги, файлы до 4 ГиБ |
| **FAT16** | да | да | тот же драйвер; выбирается по числу кластеров |
...
**FAT32/FAT16** — полноценный драйвер. Он читает и создаёт длинные имена в
...
FSInfo и создаёт каталоги с записями `.` и `..`. Тома, записанные KvantOS,
проходят `fsck.vfat` без единого замечания и нормально монтируются в Linux и
Windows.
...
| | |
|---|---|
| ![Две панели: FAT32 и NTFS](screenshots/47-file-manager.png) | ![Просмотр файла](screenshots/48-file-preview.png) |
| Две панели над настоящими томами: слева FAT32, справа NTFS | Просмотрщик: текст как текст, двоичное — дампом |
| ![Копирование с NTFS на FAT32](screenshots/49-copy-ntfs-to-fat32.png) | ![Подтверждение удаления](screenshots/50-delete-confirm.png) |
| Копирование файла с тома NTFS (только чтение) на FAT32 | Удаление всегда спрашивает подтверждение |

![Тома в консоли](screenshots/51-shell-volumes.png)
...
640×480 и др.) — в файле [AUDIT.ru.md](AUDIT.ru.md).
