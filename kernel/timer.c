/* PIT (канал 0) + системные часы + PC-спикер */
#include "kernel.h"
...
static void timer_cb(registers_t *r) {
    (void)r;
    ticks++;
    kbd_poll();          /* страховка на случай неработающего IRQ1 */
    schedule();          /* вытесняющая многозадачность */
}
...
void timer_init(u32 hz) {
    /* нулевая или заоблачная частота уронила бы деление и PIT */
    if (hz < 19) hz = 19;              /* ниже - делитель не влезает в 16 бит */
...
void beep(u32 freq, u32 ms) {
    if (freq < 19 || freq > 20000) return;    /* вне слышимого/допустимого */
